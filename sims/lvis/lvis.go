...
func (ss *Sim) ConfigStats() {
	...
	superLays := net.LayersByType(axon.SuperLayer)
	...
	ss.AddStatStd(axon.StatLayerActGe(ss.Stats, net, Train, Trial, Run, laysAll...))

	laysAdapt := net.LayersByType(axon.SuperLayer, axon.CTLayer, axon.TargetLayer)
	ss.AddStatStd(axon.StatLayerGiMult(ss.Stats, net, Train, Epoch, Run, laysAdapt...))
	...
	ss.AddStatStd(axon.StatLayerState(ss.Stats, net, Test, Trial, true, "ActM", "Output"))
}
...
func (ss *Sim) ConfigGUI(b tree.Node) {
	...
	nv.Settings.LayerNameSize = 0.03
	nv.SetNet(ss.Net)
