...
type Params struct {
	...
	Bg       float32 `viewif:"On" min:"0" desc:"background level of inhibition, which serves as a minimum inhibition level that is always present -- this is applied prior to the Gi multiplier, on the sum of computed FF + FB values"`
