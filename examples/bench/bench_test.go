...
func BenchmarkNeuronFun1Thread(b *testing.B) {
	net := &axon.Network{}
	...
	for i := 0; i < b.N; i++ {
		...
		net.NeuronFun(func(ly axon.AxonLayer, ni int, nrn *axon.Neuron) { ly.CycleNeuron(ni, nrn, ltime) },
			"CycleNeuron", false, false)
	}
}

func BenchmarkNeuronFun4Threads(b *testing.B) {
	net := &axon.Network{}
	ConfigNet(net, 4, 2048, false)
	net.InitWts()
	ltime := axon.NewTime()
	...
	for i := 0; i < b.N; i++ {
		ltime.NewState("Train")
		// wait is not currently used I think
		net.NeuronFun(func(ly axon.AxonLayer, ni int, nrn *axon.Neuron) { ly.CycleNeuron(ni, nrn, ltime) },
