...
func (ev *Env) ConfigEnv(di int) {
	...
	// defaults
	usperm := ev.Rand.Perm(cfg.NUSs, -1)
	for i := range ev.Arms {
		arm := &Arm{Length: 4, US: usperm[i%cfg.NUSs]}
		ev.Arms[i] = arm
		arm.Effort.Set(1, 1)
	}
	...
	for i := range ev.USs {
		...
		us.CSProbs[i%cfg.NCSs] = 1 // uniform allocation
	}

	ev.States = make(map[string]*etensor.Float32)
	ev.States["CS"] = etensor.NewFloat32([]int{cfg.Params.NYReps, cfg.NCSs}, nil, nil)
	ev.States["Pos"] = etensor.NewFloat32([]int{cfg.Params.NYReps, ev.MaxLength}, nil, nil)
