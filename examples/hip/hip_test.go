...
func RunPerfTest(t *testing.T, gpu bool, ndata int) {
	...
	runTable := sim.Logs.Table(etime.Train, etime.Run)
	for _, expected := range expectedVals {
		val := runTable.CellFloat(expected.name, runTable.Rows-1)
