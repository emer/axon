...
type CaNState struct {
	Cyt CaNCaMVars `desc:"in cytosol"`
	PSD CaNCaMVars `desc:"in PSD"`
}
...
func (cp *CaNParams) Defaults() {
	...
	cp.CaNCaM.SetSec(0.83333, 0.04) // 1: 40 μM-1
	cp.CaCaN01.SetSec(0.41667, 1.0) // 2: 20 μM-1
	cp.CaCaN12.SetSec(0.20833, 2.0) // 3: 10 μM-1
}
...
func (cp *CaNParams) Step(c, n *CaNState, cCa float32, nCa *float32, cCaM, nCaM *CaMKIIState) {
	*n = *c
