...
// Modes are the looping mode states for running and statistics.
...
// StatsPhase is the phase of stats processing for given mode, time.
...
type Sim struct {
	...
	StatFuncs []func(lmode Modes, ltime Times, lphase StatsPhase) `display:"-"`
...
func (ss *Sim) ConfigLoops() {
	...
	ls.AddStack(Train, Trial).
		AddTime(Run, ss.Config.Run.NRuns).
		AddTime(Epoch, ss.Config.Run.NEpochs).
		AddTimeIncr(Trial, trls, ss.Config.Run.NData).
		AddTime(Cycle, 200)
	...
	for m, _ := range ls.Stacks {
		stack := ls.Stacks[m]
		stack.Loops[Trial].OnStart.Add("ApplyInputs", func() {
			ss.ApplyInputs()
		})
	}
...
func (ss *Sim) ApplyInputs() {
	net := ss.Net
	ctx := net.Context()
	ev := ss.Envs.ByMode(ss.CurrentMode()).(*env.FixedTable)
...
func (ss *Sim) AddStat(f func(lmode Modes, ltime Times, lphase StatsPhase)) {
...
func (ss *Sim) StatsStart(lmd, ltm enums.Enum) {
	...
	ss.RunStats(lmode, ltime-1, Start)
}
...
// RunStats runs the StatFuncs for given mode, time and phase.
func (ss *Sim) RunStats(lmode Modes, ltime Times, lphase StatsPhase) {
...
func (ss *Sim) ConfigLogs() {
	...
	// ss.Logs.AddStatIntNoAggItem(AllModes, Trial, "Di")
	// ss.Logs.AddStatStringItem(AllModes, AllTimes, "RunName")
	// ss.Logs.AddStatStringItem(AllModes, Trial, "TrialName")
	//
