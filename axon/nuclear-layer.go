...
type IOParams struct {
	...
	// InhibGain multiplies the delayed, integrated inhibition input
	// that is stored in GaD and integrated through the NeuronTrace values.
	// The integration causes this value to be lower than the driving
	// inhibitory input from the CNiIO neurons.
	InhibGain float32 `default:"1.5" min:"0"`
...
func (ly *LayerParams) CNeLearn(ctx *Context, lpi, pi, ni, di uint32) {
	...
	// ioi := uint32(ly.Nuclear.IOLayIndex)
	// ioly := GetLayers(ioi)
	// Neurons[ni, di, MinusCycle] = Neurons[ioly.Indexes.NeurSt+lni, di, MinusCycle]
