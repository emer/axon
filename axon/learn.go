...
type LearnTimingParams struct {
	...
	// This is generally beneficial but not for deep layers such as PFC.
	EnableAtEnd slbool.Bool `default:"true"`

	// MinusWindow is the number of cycles (ms) for the minus phase
	// peak-finding window: the highest [TimeDiff] value within this window
	// becomes the minus-phase peak, setting [MinusPeak] and [MinusCycles],
	// and starting the window for [EnableCycles] to detect if the CaD is above
	// threshold. Generally keep this at default and adjust LearnCycles for longer
	// or shorter windows.
	MinusWindow int32 `default:"120"`
	...
	// more transient blips. Set to 0 to disable, which is needed for deeper layers
	// with more persistent activity.
	NUps int32 `default:"18,0"`
	...
	MaxUpGap int32 `default:"2"`

	// EnableWindow is the number of cycles (ms) relative to the minus phase
	// peak, [MinusCycles], to check if the CaD level gets above the LearnThr
	// threshold, at which point [EnableCycles] is set. The default of 40 is
	// is typically best.
	...
	// prior [Enabled] event, which is then saved into [EnabledPrev], thus
	// going back to the plus phase before the start of the current minus phase.
	LearnCycles int32 `default:"30,50-30"`
...
func (lt *LearnTimingParams) LearnTiming(ctx *Context, ni, di uint32) {
	...
	gotMinus := minusCyc > 0 && ctx.CyclesTotal-minusCyc <= minusThr
	justGotMinus := false

	if !gotMinus {
		newPeak := timeDiff > timePeak
		if lt.NUps > 0 {
			...
			if pups < lt.NUps {
				...
				if ctx.CyclesTotal-peakCyc >= lt.MaxUpGap || !newPeak {
					Neurons.Set(timeDiff, int(ni), int(di), int(TimePeak))
	...
	refactory := lt.Refractory.IsTrue() && learnNow > 0 // if any learning has happened before
	...
	minusWinCyc := int32(Neurons.Value(int(ni), int(di), int(MinusWindow)))
	enabled := int32(Neurons.Value(int(ni), int(di), int(Enabled)))
	enabWin := enabled - minusWinCyc
...
func (lt *LearnTimingParams) LearnRecvTrace(ctx *Context, ni, di uint32) {
	...
	ppOff := ctx.ThetaStart + ctx.ISICycles + ctx.MinusCycles + 10 // have to constrain due to RecvTrace recording
	// this is saved in advance of knowing when learnNow will happen!
	// and must be when Enabled will be relevant.
	enabled := int32(Neurons.Value(int(ni), int(di), int(Enabled)))
	if enabled == 0 {
		return
	}
	bin := ctx.CyclesTotal - ppOff
...
type LearnNeuronParams struct {
	...
	// algorithm learning, based on detecting the first major peak of
	// differential fast - slow activity associated with the start of
	// the minus phases: [TPeakCycle]. Learning occurs a fixed number of
...
func (ln *LearnNeuronParams) InitNeuronCa(ctx *Context, ni, di uint32) {
	...
	Neurons.Set(0, int(ni), int(di), int(TimePeak))
	Neurons.Set(0.0, int(ni), int(di), int(TPeakCycle))
	...
	Neurons.Set(0, int(ni), int(di), int(MinusWindow))
