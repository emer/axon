...
func (ly *LayerParams) MatrixPostPlus(ctx *Context) {
	...
	for di := uint32(0); di < ctx.NData; di++ {
		...
		for spi := uint32(1); spi < ly.Indexes.NPools; spi++ {
			pfact := PoolAvgMax(AMCaPMax, AMCycle, Avg, pf.PoolIndex(spi), di) // todo: right var?
			pfnet := ly.Striatum.BasePF + pfact
			ptD1act := PoolAvgMax(AMCaD, AMCycle, Avg, patchD1.PoolIndex(spi), di)
			ptD2act := PoolAvgMax(AMCaD, AMCycle, Avg, patchD2.PoolIndex(spi), di)
