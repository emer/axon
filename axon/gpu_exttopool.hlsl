...
void GeExtToPool(uint ni, inout Neuron nrn, in Time ctime) {
	GeExtToPool2(Layers[nrn.LayIdx], Pools[nrn.SubPoolG], ni, nrn, ctime);
}
...
void main(uint3 idx : SV_DispatchThreadID) {
	...
	if(idx.x < ns) {
		GeExtToPool(idx.x, Neurons[idx.x], CTime[0]);
	}
}
