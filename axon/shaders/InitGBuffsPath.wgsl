	...
	for (var ri = u32(0); ri < rnn; ri++) {
		for (var di = u32(0); di < maxd; di++) {
			for (var dl = u32(0); dl < mdel; dl++) {
				PathGBuf[IndexI323D(PathGBuf[0], PathGBuf[1], PathGBuf[2], u32(npst + ri),u32(di),u32(dl))] = 0;
			}
			PathGSyns[IndexF322D(PathGSyns[0], PathGSyns[1], u32(npst + ri),u32(di))] = 0.0;
		}
	}
}
