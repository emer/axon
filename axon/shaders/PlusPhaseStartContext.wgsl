...
//////// import: "act-net.go"
fn PlusPhaseStartContext(i: u32) { //gosl:kernel read-write:Ctx
	if (i != 0) {
		return;
	}
	var ctx = Ctx[0];
	Context_PlusPhaseStart(&ctx);
	Ctx[0] = ctx;
}
...
fn Context_PlusPhaseStart(ctx: ptr<function,Context>) {
	(*ctx).PhaseCycle = i32(0);
	(*ctx).MinusPhase = i32(0);
	(*ctx).PlusPhase = i32(1);
}
