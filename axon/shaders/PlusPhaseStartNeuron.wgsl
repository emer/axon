...
fn LayerParams_PlusPhaseStartNeuron(ly: ptr<function,LayerParams>, ctx: ptr<function,Context>, ni: u32,di: u32) {
	if (NeuronHasFlag(NeuronHasTarg, ni, di)) { // will be clamped in plus phase
		Neurons[Index3D(TensorStrides[60], TensorStrides[61], TensorStrides[62], u32(ni), u32(di), u32(Ext))] = Neurons[Index3D(TensorStrides[60], TensorStrides[61], TensorStrides[62], u32(ni), u32(di), u32(Target))];
		NeuronSetFlag(NeuronHasExt, ni, di);
		Neurons[Index3D(TensorStrides[60], TensorStrides[61], TensorStrides[62], u32(ni), u32(di), u32(ISI))] = -1.0;
		Neurons[Index3D(TensorStrides[60], TensorStrides[61],
		TensorStrides[62], u32(ni), u32(di), u32(ISIAvg))] = -1.0;
		Neurons[Index3D(TensorStrides[60], TensorStrides[61], TensorStrides[62], u32(ni), u32(di), u32(ActInt))] = (*ly).Acts.Init.Act;
	}
}
