...
///////////// import: "learn-net.go"
fn DWtSubMeanPath(pti: u32) { //gosl:kernel
	var ctx = Ctx[0];
	var paths=Paths[pti]; PathParams_DWtSubMean(&paths, &ctx, pti);
	Ctx[0] = ctx;
}

///////////// import: "learn-path.go"
fn PathParams_DWtSubMean(pt: ptr<function,PathParams>, ctx: ptr<function,Context>, pti: u32) {
	if ((*pt).Learn.Learn == 0) {
		return;
	}
	...
	var ri = (*pt).Indexes.RecvLayer;
	var lni = ri - (*pt).Indexes.RecvNeurSt;
	var cni = (*pt).Indexes.RecvConSt + lni;
	var synn = i32((*pt).Indexes.RecvSynSt + PathRecvCon[IndexU322D(PathRecvCon[0], PathRecvCon[1], u32(StartNN),u32(cni))]);
	if (synn < 1) {
		return;
	}
	var synst = (*pt).Indexes.RecvSynSt + PathRecvCon[IndexU322D(PathRecvCon[0], PathRecvCon[1], u32(StartOff),u32(cni))];
	var sumDWt = f32(0);
	var nnz = 0; // non-zero
