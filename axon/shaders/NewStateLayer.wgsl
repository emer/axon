...
fn LayerParams_NewStateLayer(ly: LayerParams, ctx: Context) {
...
fn LayerParams_NewStatePool(ly: LayerParams, ctx: Context, pi: u32,di: u32) {
	PoolsInt[Index3D(TensorStrides[140], TensorStrides[141], TensorStrides[142], u32(pi), u32(di), u32(Clamped))] = 0;
	if (ly.Acts.Clamp.Add == 0 && LayerParams_IsInput(ly)) {
...
//////// import: "act-net.go"
fn NewStateLayer(li: u32) { //gosl:kernel
	let ctx = Ctx[0];
	if (li >= NetworkIxs[0].NLayers) {
		return;
	}
	let layers=Layers[li]; LayerParams_NewStateLayer(layers, ctx);
}
