...
//////// import: "act-layer.go"
fn LayerParams_PlusPhaseEndPool(ly: LayerParams, ctx: Context, pi: u32,di: u32) {
	PoolCycleToPlus(pi, di);
}

//////// import: "act-net.go"
fn PlusPhaseEndPool(i: u32) { //gosl:kernel
	let ctx = Ctx[0];
	...
	let layers=Layers[li]; LayerParams_PlusPhaseEndPool(layers, ctx, pi, di);
}
...
fn PoolCycleToPlus(pi: u32,di: u32) {
	for (var vr=0; vr<AMAvgDif; vr++) { // don't do AvgDif
		Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(AvgMaxVarIndex(vr, AMPlus, Avg)))] = Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(AvgMaxVarIndex(vr, AMCycle, Avg)))];
		Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(AvgMaxVarIndex(vr, AMPlus, Max)))] = Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(AvgMaxVarIndex(vr, AMCycle, Max)))];
	}
}
