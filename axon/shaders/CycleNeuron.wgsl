...
fn LearnTimingParams_LearnTiming(lt: LearnTimingParams, ctx: Context, ni: u32,di: u32) {
	var learnNow = f32(0);
	var cadiff = Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiff))];
	var cadavg = Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffAvg))];
	cadavg += lt.CaDiffDt * (abs(cadiff) - cadavg);
	Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffAvg))] = cadavg;
	var peak = Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffPeak))];
	var peakCyc = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffPeakCyc))]);
	var newPeak = false;
	if (cadavg > lt.Threshold && cadavg > peak) {
		newPeak = true;
		peak = cadavg;
		peakCyc = ctx.CyclesTotal;
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffPeak))] = peak;
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(CaDiffPeakCyc))] = f32(peakCyc);
	}
	...
	if (pcyc > 0) {
		...
		if (mcy == lt.MinusCycles) {
			...
			u32(ni), u32(di), u32(CaDiffPeak))] = cadavg;
