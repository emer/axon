...
fn LearnTimingParams_LearnTimingPeak(lt: LearnTimingParams, ctx: Context, timeDiff: f32, ni: u32,di: u32) -> bool {
	var pups = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PeakUps))]);
	var peak = Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimePeak))];
	var peakCyc = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimeCycle))]);
	var newPeak = timeDiff > peak;
	if (ctx.CyclesTotal-peakCyc >= lt.MaxUpGap) {
		peak -= lt.DecayDt * peak;
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimePeak))] = peak;
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PeakUps))] = 0.0;
	}
	if (!newPeak) {
		return false;
	}
	...
	Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimePeak))] = peak;
	Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimeCycle))] = f32(peakCyc);
	pups++;
	if (pups >= lt.NUps) {
		...
		LearnTimingParams_TimingReset(lt, ctx, ni, di);return true;
	}
	Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PeakUps))] = f32(pups);return false;
}
fn LearnTimingParams_LearnTiming(lt: LearnTimingParams, ctx: Context, ni: u32,di: u32) -> bool {
	...
	var gotPeak = minusCyc > 0 && ctx.CyclesTotal-minusCyc <= enabledThr;
	if (!gotPeak) {
		gotPeak = LearnTimingParams_LearnTimingPeak(lt, ctx, timeDiff, ni, di);
		if (!gotPeak) {
			return false;
		}
		minusCyc = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72],
