...
fn LayerParams_SpecialPostGs(ly: LayerParams, ctx: Context, ni: u32,di: u32, saveVal: f32) {
	...
	case DSMatrixLayer: {
		var pi = LayerParams_PoolIndex(ly, NeuronIxs[Index2D(TensorStrides[10], TensorStrides[11], u32(ni), u32(NrnSubPool))]);
		var dad1 = Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(DAD1))];
		var dad2 = Pools[Index3D(TensorStrides[130], TensorStrides[131], TensorStrides[132], u32(pi), u32(di), u32(DAD2))];
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PoolDAD1))] = dad1;
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PoolDAD2))] = dad2;
		if (GlobalScalars[Index2D(TensorStrides[100], TensorStrides[101], u32(GvHasRew), u32(di))] > 0) {
			LayerParams_GNeuroMod(ly, ctx, ni, di);
		} else {
			var nda = ly.DSMatrix.PatchBurstGain*dad1 - dad2;
...
fn LearnTimingParams_LearnTiming(lt: LearnTimingParams, ctx: Context, ni: u32,di: u32) {
	...
	var gotMinus = minusCyc > 0 && ctx.CyclesTotal-minusCyc <= minusThr;
	var justGotMinus = false;
	if (!gotMinus) {
		var newPeak = timeDiff > timePeak;
		if (lt.NUps > 0) {
			...
			if (pups < lt.NUps) {
				...
				if (ctx.CyclesTotal-peakCyc >= lt.MaxUpGap || !newPeak) {
					Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(TimePeak))] = timeDiff;
					Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PeakUps))] = 0.0;return;
				}
				if (newPeak) {
					pups++;
					Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(PeakUps))] = f32(pups);
					if (pups < lt.NUps) {
						return;
					}
				}
		...
		if (ctx.CyclesTotal-peakCyc < lt.MinusWindow) {
			return;
		}
		minusCyc = peakCyc;
	...
	var refactory = lt.Refractory == 1 && learnNow > 0; // if any learning has happened before
	if (!learnImmed && justGotMinus) { // learn on previous enabled state
		...
		if (isPrevEnabled) {
			...
			if (lnow < ppOff) {
				lnow = ppOff; // can't go back before that!
			}
			if (lnow-ppOff >= 60) { // min limit on bins
				lnow = ppOff + 59;
			}
	...
	var minusWinCyc = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(MinusWindow))]);
	var enabled = i32(Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72], u32(ni), u32(di), u32(Enabled))]);
	var enabWin = enabled - minusWinCyc;
	...
	if (learnImmed) { // set LearnNow immediately when enabled, in the future
		...
		if (lnow <= ctx.ThetaStart) {
			lnow = ctx.ThetaStart + 1;
		}
		Neurons[Index3D(TensorStrides[70], TensorStrides[71], TensorStrides[72],
