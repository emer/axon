...
fn PoolCycleToPlus(pi: u32,di: u32) {
	for (var vr=0; vr<AMAvgDif; vr++) { // don't do AvgDif
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPlus, Avg)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMCycle, Avg)),u32(pi),u32(di))];
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPlus, Max)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMCycle, Max)),u32(pi),u32(di))];
	}
}
