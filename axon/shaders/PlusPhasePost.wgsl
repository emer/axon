	...
	if ((*ly).Type == PTMaintLayer && (*ly).CT.OFCposPT == 1) {
		for (var spi = u32(1); spi < np; spi++) {
			for (var di = u32(0); di < (*ctx).NData; di++) {
				...
				var val = PoolAvgMax(AMCaSpkD, AMCycle, Avg, pi, di);
...
fn LayerParams_PlusPhaseActAvg(ly: ptr<function,LayerParams>, ctx: ptr<function,Context>) {
	...
	for (var lni = u32(0); lni < nn; lni++) {
		...
		var avgSum = f32(0);
		for (var di = u32(0); di < (*ctx).NData; di++) {
			dTrgSum += LayerParams_LearnTrgAvgErrLRate(ly) * (Neurons[IndexF323D(Neurons[0], Neurons[1], Neurons[2], u32(ni),u32(di),u32(CaSpkP))] - Neurons[IndexF323D(Neurons[0], Neurons[1], Neurons[2], u32(ni),u32(di),u32(CaSpkD))]);
