...
fn LayerParams_MinusPhasePool(ly: ptr<function,LayerParams>, ctx: ptr<function,Context>, pi: u32,di: u32) {
	...
	if ((*ly).Acts.Clamp.Add == 0 && (*ly).Acts.Clamp.IsTarget == 1) {
		PoolsInt[IndexI323D(PoolsInt[0], PoolsInt[1], PoolsInt[2], u32(Clamped),u32(pi),u32(di))] = 1;
	}
...
fn LayerParams_AvgGeM(ly: ptr<function,LayerParams>, ctx: ptr<function,Context>, di: u32, geIntMinusMax: f32,giIntMinusMax: f32) {
	var gem = LayerStates[IndexF323D(LayerStates[0], LayerStates[1], LayerStates[2], u32(LayerAvgMaxGeM),u32((*ly).Index),u32(di))];
	var gim = LayerStates[IndexF323D(LayerStates[0], LayerStates[1], LayerStates[2], u32(LayerAvgMaxGiM),u32((*ly).Index),u32(di))];
	gem += (*ly).Acts.Dt.LongAvgDt * (geIntMinusMax - gem);
	gim += (*ly).Acts.Dt.LongAvgDt * (giIntMinusMax - gim);
	LayerStates[IndexF323D(LayerStates[0], LayerStates[1], LayerStates[2], u32(LayerAvgMaxGeM),u32((*ly).Index),u32(di))] = gem;
	LayerStates[IndexF323D(LayerStates[0], LayerStates[1], LayerStates[2], u32(LayerAvgMaxGiM),u32((*ly).Index),u32(di))] = gim;
}
...
fn PoolCycleToMinus(pi: u32,di: u32) {
	for (var vr=0; vr<AMAvgDif; vr++) { // don't do AvgDif
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMMinus, Avg)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMCycle, Avg)),u32(pi),u32(di))];
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMMinus, Max)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMCycle, Max)),u32(pi),u32(di))];
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPrev, Avg)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPlus, Avg)),u32(pi),u32(di))];
		Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPrev, Max)),u32(pi),u32(di))] = Pools[IndexF323D(Pools[0], Pools[1], Pools[2], u32(AvgMaxVarIndex(vr, AMPlus, Max)),u32(pi),u32(di))];
	}
}
