...
func (pt *PathParams) GatherSpikes(ctx *Context, ly *LayerParams, ni, di, lni uint32) {
	...
	gRaw := pt.Com.FloatFromGBuf(PathGBuf[bi])
	PathGBuf[bi] = 0
	gsi := lni*maxd + di
	pt.GatherSpikesGSyn(ctx, ly, ni, di, gRaw, &PathGSyns[pt.Indexes.GSynSt+gsi])
}
...
func (pt *PathParams) SendSpike(ctx *Context, ni, di, lni uint32) {
	...
	if pt.PathType == CTCtxtPath {
		if ctx.Cycle != ctx.ThetaCycles-1-int(pt.Com.DelLen) {
			return
		}
