...
func (ly *Layer) GiFromSpikes(ctx *Context) {
	...
	for lni := uint32(0); lni < nn; lni++ {
		...
		for di := uint32(0); di < ctx.NetIndexes.NData; di++ {
			...
			pl.Inhib.RawIncrInt(Neurons.Value(int(Spike), int(ni), int(di)), Neurons.Value(int(GeRaw), int(ni), int(di)), Neurons.Value(int(GeExt), int(ni), int(di)), pl.NNeurons())
			pl.AvgMaxUpdate(ctx, ni, di)
			if hasSubPools { // update layer too -- otherwise pl == lpl
				lpl := ly.Pool(0, di)
				lpl.Inhib.RawIncrInt(Neurons.Value(int(Spike), int(ni), int(di)), Neurons.Value(int(GeRaw), int(ni), int(di)), Neurons.Value(int(GeExt), int(ni), int(di)), lpl.NNeurons())
...
func (ly *Layer) PulvinarDriver(ctx *Context, lni, di uint32) (drvGe, nonDrivePct float32) {
	...
	burst := Neurons.Value(int(Burst), int(uint32(dly.NeurStIndex)+lni), int(di))
