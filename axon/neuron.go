...
type Neuron struct {
	...
	SpkMaxCa float32 `desc:"Ca integrated like CaSpkP but only starting at MaxCycStart cycle, to prevent inclusion of carryover spiking from prior theta cycle trial -- the PTau time constant otherwise results in significant carryover.  This is the input to SpkMax"`
	SpkMax   float32 `desc:"maximum CaSpkP across one theta cycle time window (max of SpkMaxCa) -- used for specialized algorithms that have more phasic behavior within a single trial, e.g., BG Matrix layer gating.  Also useful for visualization of peak activity of neurons."`
	...
	GeIntMax  float32 `desc:"maximum GeInt value across one theta cycle time window."`
	...
	GModRaw   float32 `desc:"raw modulatory conductance, received from GType = ModulatoryG projections"`
	GModSyn   float32 `desc:"syn integrated modulatory conductance, received from GType = ModulatoryG projections"`
	GMaintRaw float32 `desc:"raw maintenance conductance, received from GType = MaintG projections"`
	GMaintSyn float32 `desc:"syn integrated maintenance conductance, integrated using MaintNMDA params."`
	...
	GnmdaMaint float32 `desc:"net postsynaptic maintenance NMDA conductance, computed from GMaintSyn and GMaintRaw, after Mg V-gating and Gbar -- added directly to Ge as it has the same reversal potential"`
	...
	SKCaIn float32 `desc:"intracellular calcium store level, available to be released with spiking as SKCaR, which can bind to SKCa receptors and drive K current. replenishment is a function of spiking activity being below a threshold"`
