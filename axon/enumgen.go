...
var _NeuronVarsValueMap = map[string]NeuronVars{`Spike`: 0, `Spiked`: 1, `Act`: 2, `ActInt`: 3, `Ge`: 4, `Gi`: 5, `Gk`: 6, `Inet`: 7, `Vm`: 8, `VmDend`: 9, `ISI`: 10, `ISIAvg`: 11, `Ext`: 12, `Target`: 13, `CaM`: 14, `CaP`: 15, `CaD`: 16, `CaDPrev`: 17, `CaSyn`: 18, `LearnCa`: 19, `LearnCaM`: 20, `LearnCaP`: 21, `LearnCaD`: 22, `CaDiff`: 23, `GaM`: 24, `GaP`: 25, `GaD`: 26, `TimeDiff`: 27, `TimePeak`: 28, `TPeakCycle`: 29, `MinusPeak`: 30, `MinusCycle`: 31, `MinusWindow`: 32, `Enabled`: 33, `EnabledPrev`: 34, `LearnNow`: 35, `RLRate`: 36, `ETrace`: 37, `ETrLearn`: 38, `GnmdaSyn`: 39, `Gnmda`: 40, `GnmdaLrn`: 41, `GnmdaMaint`: 42, `NmdaCa`: 43, `Gvgcc`: 44, `VgccM`: 45, `VgccH`: 46, `VgccCa`: 47, `VgccCaInt`: 48, `Burst`: 49, `BurstPrv`: 50, `CtxtGe`: 51, `CtxtGeRaw`: 52, `CtxtGeOrig`: 53, `GgabaB`: 54, `GababM`: 55, `GababX`: 56, `Gak`: 57, `SSGiDend`: 58, `GknaMed`: 59, `GknaSlow`: 60, `Gkir`: 61, `KirM`: 62, `Gsk`: 63, `SKCaIn`: 64, `SKCaR`: 65, `SKCaM`: 66, `Gmahp`: 67, `MahpN`: 68, `Gsahp`: 69, `SahpCa`: 70, `SahpN`: 71, `ActM`: 72, `ActP`: 73, `Beta1`: 74, `Beta2`: 75, `CaPMax`: 76, `CaPMaxCa`: 77, `GeNoise`: 78, `GeNoiseP`: 79, `GiNoise`: 80, `GiNoiseP`: 81, `GeExt`: 82, `GeRaw`: 83, `GeSyn`: 84, `GiRaw`: 85, `GiSyn`: 86, `GeInt`: 87, `GeIntNorm`: 88, `GiInt`: 89, `GModRaw`: 90, `GModSyn`: 91, `SMaintP`: 92, `GMaintRaw`: 93, `GMaintSyn`: 94, `NeurFlags`: 95, `NeuronTraces`: 96}

var _NeuronVarsDescMap = map[NeuronVars]string{0: `Spike is whether neuron has spiked or not on this cycle (0 or 1).`, 1: `Spiked is 1 if neuron has spiked within the last 10 cycles (msecs), corresponding to a nominal max spiking rate of 100 Hz, 0 otherwise. Useful for visualization and computing activity levels in terms of average spiked levels.`, 2: `Act is rate-coded activation value reflecting instantaneous estimated rate of spiking, based on 1 / ISIAvg. It is integrated over time for ActInt which is then used for performance statistics and layer average activations, etc. Should not be used for learning or other computations: just for stats / display.`, 3: `ActInt is integrated running-average activation value computed from Act with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall activation state across the ThetaCycle time scale, as the overall response of network to current input state. This is copied to ActM and ActP at the ends of the minus and plus phases, respectively, and used in computing some performance-level statistics (based on ActM). Should not be used for learning or other computations.`, 4: `Ge is total excitatory conductance, including all forms of excitation (e.g., NMDA). Does *not* include the Gbar.E factor.`, 5: `Gi is total inhibitory synaptic conductance, i.e., the net inhibitory input to the neuron. Does *not* include the Gbar.I factor.`, 6: `Gk is total potassium conductance, typically reflecting sodium-gated potassium currents involved in adaptation effects. Does *not* include the Gbar.K factor.`, 7: `Inet is net current produced by all channels, which drives update of Vm.`, 8: `Vm is the membrane potential at the cell body, which integrates Inet current over time, and drives spiking at the axon initial segment of the neuron.`, 9: `VmDend is the dendritic membrane potential, which has a slower time constant than Vm and is not subject to the VmR reset after spiking.`, 10: `ISI is the current inter-spike-interval, which counts up since last spike. Starts at -1 when initialized.`, 11: `ISIAvg is the average inter-spike-interval, i.e., the average time interval between spikes, integrated with ISITau rate constant (relatively fast) to capture something close to an instantaneous spiking rate. Starts at -1 when initialized, and goes to -2 after first spike, and is only valid after the second spike post-initialization.`, 12: `Ext is the external input: drives activation of unit from outside influences (e.g., sensory input).`, 13: `Target is the target value: drives learning to produce this activation value.`, 14: `CaM is the spike-driven calcium trace at the neuron level, which then drives longer time-integrated variables: [CaP] and [CaD]. These variables are used for statistics and display to capture spiking activity at different timescales. They fluctuate more than [Act] and [ActInt], but are closer to the biological variables driving learning. CaM is the exponential integration of SpikeG * Spike using the MTau time constant (typically 5), and simulates a calmodulin (CaM) like signal, at an abstract level.`, 15: `CaP is the continuous cascaded integration of [CaM] using the PTau time constant (typically 40), representing a neuron-level, purely spiking version of the plus, LTP direction of weight change in the Kinase learning rule, dependent on CaMKII. This is not used for learning (see [LearnCaP]), but instead for statistics as a representation of recent activity.`, 16: `CaD is the continuous cascaded integration [CaP] using the DTau time constant (typically 40), representing a neuron-level, purely spiking version of the minus, LTD direction of weight change in the Kinase learning rule, dependent on DAPK1. This is not used for learning (see [LearnCaD]), but instead for statistics as a representation of trial-level activity.`, 17: `CaDPrev is the final [CaD] activation state at the end of previous theta cycle. This is used for specialized learning mechanisms that operate on delayed sending activations.`, 18: `CaSyn is the neuron-level integration of spike-driven calcium, used to approximate synaptic calcium influx as a product of sender and receiver neuron CaSyn values, which are integrated separately because it is computationally much more efficient. CaSyn enters into a Sender * Receiver product at each synapse to give the effective credit assignment factor for learning. This value is driven directly by spikes, with an exponential integration time constant of 30 msec (default), which captures the coincidence window for pre*post firing on NMDA receptor opening. The neuron [NeuronTraces] values record the temporal trajectory of CaSyn over the course of the theta cycle window, and then the pre*post product is integrated over these bins at the synaptic level.`, 19: `LearnCa is the receiving neuron calcium signal, which is integrated up to [LearnCaP] and [LearnCaD], the difference of which is the temporal error component of the kinase cortical learning rule. LearnCa combines NMDA via [NmdaCa] and spiking-driven VGCC [VgccCaInt] calcium sources. The NMDA signal reflects both sending and receiving activity, while the VGCC signal is purely receiver spiking, and a balance of both works best.`, 20: `LearnCaM is the integrated [LearnCa] at the MTau timescale (typically 5), simulating a calmodulin (CaM) like signal, which then drives [LearnCaP], and [LearnCaD] for the delta signal for error-driven learning.`, 21: `LearnCaP is the cascaded integration of [LearnCaM] using the PTau time constant (typically 40), representing the plus, LTP direction of weight change, capturing the function of CaMKII in the Kinase learning rule.`, 22: `LearnCaD is the cascaded integration of [LearnCaP] using the DTau time constant (typically 40), representing the minus, LTD direction of weight change, capturing the function of DAPK1 in the Kinase learning rule.`, 23: `CaDiff is difference between [LearnCaP] - [LearnCaD]. This is the error signal that drives error-driven learning.`, 24: `GaM is first-level integration of all input conductances g_a, which then drives longer time-integrated variables: [GaP] and [GaD]. These variables are used for timing of learning based on bursts of activity change over time: at the minus and plus phases.`, 25: `GaP is the continuous cascaded integration of [GaM] using the PTau time constant (typically 40), representing a neuron-level, all-conductance-based version of the plus, LTP direction of weight change in the Kinase learning rule.`, 26: `GaD is the continuous cascaded integration of [GaP] using the DTau time constant (typically 40), representing a neuron-level, all-conductance-based version of the minus, LTD direction of weight change in the Kinase learning rule.`, 27: `TimeDiff is the running time-average of |P - D| (absolute value), used for determining the timing of learning in terms of onsets of peaks. See [TPeakCycle]. GaP - GaD is used, as it is smoother and more reliable than LearnCaP - D.`, 28: `TimePeak is the current peak value of TimeDiff, used for computing [TPeakCycle] when [TimeDiff] &gt; [TimePeak], which in turn determines [MinusPeak] after the enabling time window has passed.`, 29: `TPeakCycle is the absolute cycle (ms, CyclesTotal) when the last [TimePeak] value was updated.`, 30: `MinusPeak is the value of the last detected minus-phase peak, from [TimePeak], This typically occurs at the onset of the minus phase, and drives the timing of learning a given number of cycles after that.`, 31: `MinusCycle is the absolute cycle (ms, CyclesTotal) when the minus-phase peak occurred, copied from [TPeakCycle] for that peak.`, 32: `MinusWindow is the absolute cycle (ms, CyclesTotal) when the minus-phase peak detection window was reached, and the minus phase was detected. After this, there are additional cycles where the neuron could get over the CaD threshold for learning, or not. LearnNow is relative to this point.`, 33: `Enabled is the absolute cycle (ms, CyclesTotal) when the receiving neuron is above threshold for learning, and a minus-phase peak has been detected. For neocortex, this is after [MinusWindow], and the neuron CaD level has gone above the learning threshold, within a minimum number of cycles. If not using flexible learning timing, this is set to the end of the theta cycle. See [LearnTimingParams] for details.`, 34: `EnabledPrev is the absolute cycle (ms, CyclesTotal) for the previous [Enabled] value, if set. This is used for learning that is triggered by a minus phase subsequent to being enabled.`, 35: `LearnNow is the absolute cycle (ms, CyclesTotal) when the receiving neuron actually learns. See [Enabled] for enabling conditions, and [LearnTimingParams] for parameters. For neocortex, this can be based on going back from the subsequent minus phase peak, after being enabled (see [EnabledPrev]).`, 36: `RLRate is recv-unit based learning rate multiplier, reflecting the sigmoid derivative computed from [CaD] of recv unit, and the normalized difference (CaP - CaD) / MAX(CaP - CaD).`, 37: `ETrace is the eligibility trace for this neuron.`, 38: `ETrLearn is the learning factor for the eligibility trace for this neuron. 1 + ETraceScale * [ETrace]`, 39: `GnmdaSyn is the integrated NMDA synaptic current on the receiving neuron. It adds GeRaw and decays with a time constant.`, 40: `Gnmda is the net postsynaptic (receiving) NMDA conductance, after Mg V-gating and Gbar. This is added directly to Ge as it has the same reversal potential.`, 41: `GnmdaLrn is learning version of integrated NMDA recv synaptic current. It adds [GeRaw] and decays with a time constant. This drives [NmdaCa] that then drives [LearnCa] for learning.`, 42: `GnmdaMaint is net postsynaptic maintenance NMDA conductance, computed from [GMaintSyn] and [GMaintRaw], after Mg V-gating and Gbar. This is added directly to Ge as it has the same reversal potential.`, 43: `NmdaCa is NMDA calcium computed from GnmdaLrn, drives learning via CaM.`, 44: `Gvgcc is conductance (via Ca) for VGCC voltage gated calcium channels.`, 45: `VgccM is activation gate of VGCC channels.`, 46: `VgccH inactivation gate of VGCC channels.`, 47: `VgccCa is the instantaneous VGCC calcium flux: can be driven by spiking or directly from Gvgcc.`, 48: `VgccCaInt is the time-integrated VGCC calcium flux. This is actually what drives learning.`, 49: `Burst is the layer 5 IB intrinsic bursting neural activation value, computed by thresholding the [CaP] value in Super superficial layers.`, 50: `BurstPrv is previous Burst bursting activation from prior time step. Used for context-based learning.`, 51: `CtxtGe is context (temporally delayed) excitatory conductance, driven by deep bursting at end of the plus phase, for CT layers.`, 52: `CtxtGeRaw is raw update of context (temporally delayed) excitatory conductance, driven by deep bursting at end of the plus phase, for CT layers.`, 53: `CtxtGeOrig is original CtxtGe value prior to any decay factor. Updates at end of plus phase.`, 54: `GgabaB is net GABA-B conductance, after Vm gating and Gk + Gbase. Applies to Gk, not Gi, for GIRK, with .1 reversal potential.`, 55: `GababM is the GABA-B / GIRK activation, which is a time-integrated value with rise and decay time constants.`, 56: `GababX is GABA-B / GIRK internal drive variable. This gets the raw activation and decays.`, 57: `Gak is the conductance of A-type K potassium channels.`, 58: `SSGiDend is the amount of SST+ somatostatin positive slow spiking inhibition applied to dendritic Vm (VmDend).`, 59: `GknaMed is the conductance of sodium-gated potassium channel (KNa) medium dynamics (Slick), which produces accommodation / adaptation.`, 60: `GknaSlow is the conductance of sodium-gated potassium channel (KNa) slow dynamics (Slack), which produces accommodation / adaptation.`, 61: `Gkir is the conductance of the potassium (K) inwardly rectifying channel, which is strongest at low membrane potentials. Can be modulated by DA.`, 62: `KirM is the Kir potassium (K) inwardly rectifying gating value.`, 63: `Gsk is Calcium-gated potassium channel conductance as a function of Gbar * SKCaM.`, 64: `SKCaIn is intracellular calcium store level, available to be released with spiking as SKCaR, which can bind to SKCa receptors and drive K current. replenishment is a function of spiking activity being below a threshold.`, 65: `SKCaR is the released amount of intracellular calcium, from SKCaIn, as a function of spiking events. This can bind to SKCa channels and drive K currents.`, 66: `SKCaM is the Calcium-gated potassium channel gating factor, driven by SKCaR via a Hill equation as in chans.SKPCaParams.`, 67: `Gmahp is medium time scale AHP conductance.`, 68: `MahpN is accumulating voltage-gated gating value for the medium time scale AHP.`, 69: `Gsahp is slow time scale AHP conductance.`, 70: `SahpCa is slowly accumulating calcium value that drives the slow AHP.`, 71: `SahpN is the sAHP gating value.`, 72: `ActM is ActInt activation state at end of third quarter, representing the posterior-cortical minus phase activation. This is used for statistics and monitoring network performance. Should not be used for learning or other computations.`, 73: `ActP is ActInt activation state at end of fourth quarter, representing the posterior-cortical plus_phase activation. This is used for statistics and monitoring network performance. Should not be used for learning or other computations.`, 74: `Beta1 is the activation state at the first beta cycle within current state processing window (i.e., at 50 msec), as saved by Beta1() function. Used for example in hippocampus for CA3, CA1 learning.`, 75: `Beta2 is the activation state at the second beta cycle within current state processing window (i.e., at 100 msec), as saved by Beta2() function. Used for example in hippocampus for CA3, CA1 learning.`, 76: `CaPMax is the maximum [CaP] across one theta cycle time window (max of CaPMaxCa). It is used for specialized algorithms that have more phasic behavior within a single trial, e.g., BG Matrix layer gating. Also useful for visualization of peak activity of neurons.`, 77: `CaPMaxCa is the Ca integrated like [CaP] but only starting at the MaxCycStart cycle, to prevent inclusion of carryover spiking from prior theta cycle trial. The PTau time constant otherwise results in significant carryover. This is the input to CaPMax.`, 78: `GeNoise is integrated noise excitatory conductance, added into Ge.`, 79: `GeNoiseP is accumulating poisson probability factor for driving excitatory noise spiking. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda as function of noise firing rate.`, 80: `GiNoise is integrated noise inhibitory conductance, added into Gi.`, 81: `GiNoiseP is accumulating poisson probability factor for driving inhibitory noise spiking. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda as a function of noise firing rate.`, 82: `GeExt is extra excitatory conductance added to Ge, from Ext input, GeCtxt etc.`, 83: `GeRaw is the raw excitatory conductance (net input) received from senders = current raw spiking drive.`, 84: `GeSyn is the time-integrated total excitatory (AMPA) synaptic conductance, with an instantaneous rise time from each spike (in GeRaw) and exponential decay with Dt.GeTau, aggregated over pathways. Does *not* include Gbar.E.`, 85: `GiRaw is the raw inhibitory conductance (net input) received from senders = current raw spiking drive.`, 86: `GiSyn is time-integrated total inhibitory synaptic conductance, with an instantaneous rise time from each spike (in GiRaw) and exponential decay with Dt.GiTau, aggregated over pathways -- does *not* include Gbar.I. This is added with computed FFFB inhibition to get the full inhibition in Gi.`, 87: `GeInt is integrated running-average activation value computed from Ge with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall Ge level across the ThetaCycle time scale (Ge itself fluctuates considerably). This is useful for stats to set strength of connections etc to get neurons into right range of overall excitatory drive.`, 88: `GeIntNorm is normalized GeInt value (divided by the layer maximum). This is used for learning in layers that require learning on subthreshold activity.`, 89: `GiInt is integrated running-average activation value computed from GiSyn with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall synaptic Gi level across the ThetaCycle time scale (Gi itself fluctuates considerably). Useful for stats to set strength of connections etc to get neurons into right range of overall inhibitory drive.`, 90: `GModRaw is raw modulatory conductance, received from GType = ModulatoryG pathways.`, 91: `GModSyn is syn integrated modulatory conductance, received from GType = ModulatoryG pathways.`, 92: `SMaintP is accumulating poisson probability factor for driving self-maintenance by simulating a population of mutually interconnected neurons. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda based on accumulating self maint factor.`, 93: `GMaintRaw is raw maintenance conductance, received from GType = MaintG pathways.`, 94: `GMaintSyn is syn integrated maintenance conductance, integrated using MaintNMDA params.`, 95: `NeurFlags are bit flags for binary state variables, which are converted to / from uint32. These need to be in Vars because they can be differential per data (for ext inputs) and are writable (indexes are read only).`, 96: `NeuronTraces is a vector of values starting here, with aggregated [CaSyn] values in time bins of [NeuronTraceCycles] across two theta cycles, for computing synaptic calcium efficiently. Each bin = Sum(CaSyn / NeuronTraceCycles). Total number of bins = 2 * [Context.ThetaCycles] / NeuronTraceCycles. Use [NeuronTraceIndex] to access. Synaptic calcium is integrated from sender * receiver NeuronTraces values, with weights for CaP vs CaD that reflect their faster vs. slower time constants, respectively. CaD is used for the credit assignment factor, while CaP - CaD is used directly for error-driven learning at Target layers.`}

var _NeuronVarsMap = map[NeuronVars]string{0: `Spike`, 1: `Spiked`, 2: `Act`, 3: `ActInt`, 4: `Ge`, 5: `Gi`, 6: `Gk`, 7: `Inet`, 8: `Vm`, 9: `VmDend`, 10: `ISI`, 11: `ISIAvg`, 12: `Ext`, 13: `Target`, 14: `CaM`, 15: `CaP`, 16: `CaD`, 17: `CaDPrev`, 18: `CaSyn`, 19: `LearnCa`, 20: `LearnCaM`, 21: `LearnCaP`, 22: `LearnCaD`, 23: `CaDiff`, 24: `GaM`, 25: `GaP`, 26: `GaD`, 27: `TimeDiff`, 28: `TimePeak`, 29: `TPeakCycle`, 30: `MinusPeak`, 31: `MinusCycle`, 32: `MinusWindow`, 33: `Enabled`, 34: `EnabledPrev`, 35: `LearnNow`, 36: `RLRate`, 37: `ETrace`, 38: `ETrLearn`, 39: `GnmdaSyn`, 40: `Gnmda`, 41: `GnmdaLrn`, 42: `GnmdaMaint`, 43: `NmdaCa`, 44: `Gvgcc`, 45: `VgccM`, 46: `VgccH`, 47: `VgccCa`, 48: `VgccCaInt`, 49: `Burst`, 50: `BurstPrv`, 51: `CtxtGe`, 52: `CtxtGeRaw`, 53: `CtxtGeOrig`, 54: `GgabaB`, 55: `GababM`, 56: `GababX`, 57: `Gak`, 58: `SSGiDend`, 59: `GknaMed`, 60: `GknaSlow`, 61: `Gkir`, 62: `KirM`, 63: `Gsk`, 64: `SKCaIn`, 65: `SKCaR`, 66: `SKCaM`, 67: `Gmahp`, 68: `MahpN`, 69: `Gsahp`, 70: `SahpCa`, 71: `SahpN`, 72: `ActM`, 73: `ActP`, 74: `Beta1`, 75: `Beta2`, 76: `CaPMax`, 77: `CaPMaxCa`, 78: `GeNoise`, 79: `GeNoiseP`, 80: `GiNoise`, 81: `GiNoiseP`, 82: `GeExt`, 83: `GeRaw`, 84: `GeSyn`, 85: `GiRaw`, 86: `GiSyn`, 87: `GeInt`, 88: `GeIntNorm`, 89: `GiInt`, 90: `GModRaw`, 91: `GModSyn`, 92: `SMaintP`, 93: `GMaintRaw`, 94: `GMaintSyn`, 95: `NeurFlags`, 96: `NeuronTraces`}
