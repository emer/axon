...
var _ValenceTypesValues = []ValenceTypes{0, 1, 2, 164}

// ValenceTypesN is the highest valid value for type ValenceTypes, plus one.
const ValenceTypesN ValenceTypes = 165

var _ValenceTypesValueMap = map[string]ValenceTypes{`Positive`: 0, `Negative`: 1, `Cost`: 2, `ValenceTypesN`: 164}

var _ValenceTypesDescMap = map[ValenceTypes]string{0: `Positive valence codes for outcomes aligned with drives / goals.`, 1: `Negative valence codes for harmful or aversive outcomes.`, 2: `Cost codes for continous ongoing cost factors such as Time and Effort`, 164: ``}

var _ValenceTypesMap = map[ValenceTypes]string{0: `Positive`, 1: `Negative`, 2: `Cost`, 164: `ValenceTypesN`}
...
var _NeuronFlagsValues = []NeuronFlags{1, 2, 4, 8, 163}

// NeuronFlagsN is the highest valid value for type NeuronFlags, plus one.
const NeuronFlagsN NeuronFlags = 164

var _NeuronFlagsValueMap = map[string]NeuronFlags{`NeuronOff`: 1, `NeuronHasExt`: 2, `NeuronHasTarg`: 4, `NeuronHasCmpr`: 8, `NeuronFlagsN`: 163}

var _NeuronFlagsDescMap = map[NeuronFlags]string{1: `NeuronOff flag indicates that this neuron has been turned off (i.e., lesioned)`, 2: `NeuronHasExt means the neuron has external input in its Ext field`, 4: `NeuronHasTarg means the neuron has external target input in its Target field`, 8: `NeuronHasCmpr means the neuron has external comparison input in its Target field -- used for computing comparison statistics but does not drive neural activity ever`, 163: ``}

var _NeuronFlagsMap = map[NeuronFlags]string{1: `NeuronOff`, 2: `NeuronHasExt`, 4: `NeuronHasTarg`, 8: `NeuronHasCmpr`, 163: `NeuronFlagsN`}
...
var _NeuronIndexVarsValues = []NeuronIndexVars{0, 1, 2, 157}

// NeuronIndexVarsN is the highest valid value for type NeuronIndexVars, plus one.
const NeuronIndexVarsN NeuronIndexVars = 158

var _NeuronIndexVarsValueMap = map[string]NeuronIndexVars{`NrnNeurIndex`: 0, `NrnLayIndex`: 1, `NrnSubPool`: 2, `NeuronIndexVarsN`: 157}
...
var _NeuronIndexVarsMap = map[NeuronIndexVars]string{0: `NrnNeurIndex`, 1: `NrnLayIndex`, 2: `NrnSubPool`, 157: `NeuronIndexVarsN`}
...
var _GPLayerTypesValues = []GPLayerTypes{0, 1, 2, 164}

// GPLayerTypesN is the highest valid value for type GPLayerTypes, plus one.
const GPLayerTypesN GPLayerTypes = 165

var _GPLayerTypesValueMap = map[string]GPLayerTypes{`GPePr`: 0, `GPeAk`: 1, `GPi`: 2, `GPLayerTypesN`: 164}

var _GPLayerTypesDescMap = map[GPLayerTypes]string{0: `GPePr is the set of prototypical GPe neurons, mediating classical NoGo`, 1: `GPeAk is arkypallidal layer of GPe neurons, receiving inhibition from GPePr and projecting inhibition to Mtx`, 2: `GPi is the inner globus pallidus, functionally equivalent to SNr, receiving from MtxGo and GPePr, and sending inhibition to VThal`, 164: ``}

var _GPLayerTypesMap = map[GPLayerTypes]string{0: `GPePr`, 1: `GPeAk`, 2: `GPi`, 164: `GPLayerTypesN`}
...
var _SynapseVarsValues = []SynapseVars{0, 1, 2, 3, 4, 156}

// SynapseVarsN is the highest valid value for type SynapseVars, plus one.
const SynapseVarsN SynapseVars = 157

var _SynapseVarsValueMap = map[string]SynapseVars{`Wt`: 0, `LWt`: 1, `SWt`: 2, `DWt`: 3, `DSWt`: 4, `SynapseVarsN`: 156}
...
var _SynapseVarsMap = map[SynapseVars]string{0: `Wt`, 1: `LWt`, 2: `SWt`, 3: `DWt`, 4: `DSWt`, 156: `SynapseVarsN`}
...
var _SynapseIndexVarsValues = []SynapseIndexVars{0, 1, 2, 154}

// SynapseIndexVarsN is the highest valid value for type SynapseIndexVars, plus one.
const SynapseIndexVarsN SynapseIndexVars = 155

var _SynapseIndexVarsValueMap = map[string]SynapseIndexVars{`SynRecvIndex`: 0, `SynSendIndex`: 1, `SynPathIndex`: 2, `SynapseIndexVarsN`: 154}

var _SynapseIndexVarsDescMap = map[SynapseIndexVars]string{0: `SynRecvIndex is receiving neuron index in network&#39;s global list of neurons`, 1: `SynSendIndex is sending neuron index in network&#39;s global list of neurons`, 2: `SynPathIndex is pathway index in global list of pathways organized as [Layers][RecvPaths]`, 154: ``}

var _SynapseIndexVarsMap = map[SynapseIndexVars]string{0: `SynRecvIndex`, 1: `SynSendIndex`, 2: `SynPathIndex`, 154: `SynapseIndexVarsN`}
