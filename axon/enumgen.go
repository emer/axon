...
var _PoolIntVarsDescMap = map[PoolIntVars]string{0: `PoolNeurSt is the starting layer-wise index within the list of neurons in this pool. Add layer starting neuron index (NeurSt) to get index into global network neurons list.`, 1: `PoolNeurEd is the ending (exclusive) layer-wise index within the list of neurons in this pool. Add layer starting neuron index (NeurSt) to get index into global network neurons list.`, 2: `PoolLayerIdx is the layer index for this pool.`, 3: `PoolIsLayer is true (&gt; 0) if this pool represents the entire layer, which is always the first pool in the list of pools for a layer.`, 4: `Clamped if true (!=0), this layer is hard-clamped and should use GeExts exclusively for PV.`, 5: `PoolGated is true (&gt; 0) if this pool gated (for [MatrixLayer], [BGThalLayer])`, 6: `FFsRawInt is the int32 atomic add compatible integration of [fsfffb.FFsRaw].`, 7: `FBsRawInt is the int32 atomic add compatible integration of [fsfffb.FBsRaw].`, 8: `GeExtRawInt is the int32 atomic add compatible integration of [fsfffb.GeExtRaw].`, 9: `PoolIntAvgMaxStart is the starting point for int32 AvgMax variables. Use AvgMaxIntVarIdx to get the relevant variable index. There are only values for Cycle phase, for the different variables.`}
