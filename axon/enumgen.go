...
var _NeuronVarsValueMap = map[string]NeuronVars{`Spike`: 0, `Spiked`: 1, `Act`: 2, `ActInt`: 3, `Ge`: 4, `Gi`: 5, `Gk`: 6, `Inet`: 7, `Vm`: 8, `VmDend`: 9, `ISI`: 10, `ISIAvg`: 11, `Ext`: 12, `Target`: 13, `CaM`: 14, `CaP`: 15, `CaD`: 16, `CaDPrev`: 17, `CaSyn`: 18, `LearnCa`: 19, `LearnCaM`: 20, `LearnCaP`: 21, `LearnCaD`: 22, `CaDiff`: 23, `CaDiffAvg`: 24, `CaDiffPeak`: 25, `CaDiffPeakCyc`: 26, `MinusPeak`: 27, `MinusPeakCyc`: 28, `PlusPeak`: 29, `PlusPeakCyc`: 30, `LearnDiff`: 31, `LearnNow`: 32, `RLRate`: 33, `ETrace`: 34, `ETraceLearn`: 35, `GnmdaSyn`: 36, `Gnmda`: 37, `GnmdaLrn`: 38, `GnmdaMaint`: 39, `NmdaCa`: 40, `Gvgcc`: 41, `VgccM`: 42, `VgccH`: 43, `VgccCa`: 44, `VgccCaInt`: 45, `Burst`: 46, `BurstPrv`: 47, `CtxtGe`: 48, `CtxtGeRaw`: 49, `CtxtGeOrig`: 50, `GgabaB`: 51, `GababM`: 52, `GababX`: 53, `Gak`: 54, `SSGiDend`: 55, `GknaMed`: 56, `GknaSlow`: 57, `Gkir`: 58, `KirM`: 59, `Gsk`: 60, `SKCaIn`: 61, `SKCaR`: 62, `SKCaM`: 63, `Gmahp`: 64, `MahpN`: 65, `Gsahp`: 66, `SahpCa`: 67, `SahpN`: 68, `ActM`: 69, `ActP`: 70, `Beta1`: 71, `Beta2`: 72, `CaPMax`: 73, `CaPMaxCa`: 74, `GeNoise`: 75, `GeNoiseP`: 76, `GiNoise`: 77, `GiNoiseP`: 78, `GeExt`: 79, `GeRaw`: 80, `GeSyn`: 81, `GiRaw`: 82, `GiSyn`: 83, `GeInt`: 84, `GeIntNorm`: 85, `GiInt`: 86, `GModRaw`: 87, `GModSyn`: 88, `SMaintP`: 89, `GMaintRaw`: 90, `GMaintSyn`: 91, `NeurFlags`: 92, `CaBins`: 93}

var _NeuronVarsDescMap = map[NeuronVars]string{0: `Spike is whether neuron has spiked or not on this cycle (0 or 1).`, 1: `Spiked is 1 if neuron has spiked within the last 10 cycles (msecs), corresponding to a nominal max spiking rate of 100 Hz, 0 otherwise. Useful for visualization and computing activity levels in terms of average spiked levels.`, 2: `Act is rate-coded activation value reflecting instantaneous estimated rate of spiking, based on 1 / ISIAvg. It is integrated over time for ActInt which is then used for performance statistics and layer average activations, etc. Should not be used for learning or other computations: just for stats / display.`, 3: `ActInt is integrated running-average activation value computed from Act with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall activation state across the ThetaCycle time scale, as the overall response of network to current input state. This is copied to ActM and ActP at the ends of the minus and plus phases, respectively, and used in computing some performance-level statistics (based on ActM). Should not be used for learning or other computations.`, 4: `Ge is total excitatory conductance, including all forms of excitation (e.g., NMDA). Does *not* include the Gbar.E factor.`, 5: `Gi is total inhibitory synaptic conductance, i.e., the net inhibitory input to the neuron. Does *not* include the Gbar.I factor.`, 6: `Gk is total potassium conductance, typically reflecting sodium-gated potassium currents involved in adaptation effects. Does *not* include the Gbar.K factor.`, 7: `Inet is net current produced by all channels, which drives update of Vm.`, 8: `Vm is the membrane potential at the cell body, which integrates Inet current over time, and drives spiking at the axon initial segment of the neuron.`, 9: `VmDend is the dendritic membrane potential, which has a slower time constant than Vm and is not subject to the VmR reset after spiking.`, 10: `ISI is the current inter-spike-interval, which counts up since last spike. Starts at -1 when initialized.`, 11: `ISIAvg is the average inter-spike-interval, i.e., the average time interval between spikes, integrated with ISITau rate constant (relatively fast) to capture something close to an instantaneous spiking rate. Starts at -1 when initialized, and goes to -2 after first spike, and is only valid after the second spike post-initialization.`, 12: `Ext is the external input: drives activation of unit from outside influences (e.g., sensory input).`, 13: `Target is the target value: drives learning to produce this activation value.`, 14: `CaM is the spike-driven calcium trace at the neuron level, which then drives longer time-integrated variables: [CaP] and [CaD]. These variables are used for statistics and display to capture spiking activity at different timescales. They fluctuate more than [Act] and [ActInt], but are closer to the biological variables driving learning. CaM is the exponential integration of SpikeG * Spike using the MTau time constant (typically 5), and simulates a calmodulin (CaM) like signal, at an abstract level.`, 15: `CaP is the continuous cascaded integration of [CaM] using the PTau time constant (typically 40), representing a neuron-level, purely spiking version of the plus, LTP direction of weight change in the Kinase learning rule, dependent on CaMKII. This is not used for learning (see [LearnCaP]), but instead for statistics as a representation of recent activity.`, 16: `CaD is the continuous cascaded integration [CaP] using the DTau time constant (typically 40), representing a neuron-level, purely spiking version of the minus, LTD direction of weight change in the Kinase learning rule, dependent on DAPK1. This is not used for learning (see [LearnCaD]), but instead for statistics as a representation of trial-level activity.`, 17: `CaDPrev is the final [CaD] activation state at the end of previous theta cycle. This is used for specialized learning mechanisms that operate on delayed sending activations.`, 18: `CaSyn is the neuron-level integration of spike-driven calcium, used to approximate synaptic calcium influx as a product of sender and receiver neuron CaSyn values, which are integrated separately because it is computationally much more efficient. CaSyn enters into a Sender * Receiver product at each synapse to give the effective credit assignment factor for learning. This value is driven directly by spikes, with an exponential integration time constant of 30 msec (default), which captures the coincidence window for pre*post firing on NMDA receptor opening. The neuron [CaBins] values record the temporal trajectory of CaSyn over the course of the theta cycle window, and then the pre*post product is integrated over these bins at the synaptic level.`, 19: `LearnCa is the receiving neuron calcium signal, which is integrated up to [LearnCaP] and [LearnCaD], the difference of which is the temporal error component of the kinase cortical learning rule. LearnCa combines NMDA via [NmdaCa] and spiking-driven VGCC [VgccCaInt] calcium sources. The NMDA signal reflects both sending and receiving activity, while the VGCC signal is purely receiver spiking, and a balance of both works best.`, 20: `LearnCaM is the integrated [LearnCa] at the MTau timescale (typically 5), simulating a calmodulin (CaM) like signal, which then drives [LearnCaP], and [LearnCaD] for the delta signal for error-driven learning.`, 21: `LearnCaP is the cascaded integration of [LearnCaM] using the PTau time constant (typically 40), representing the plus, LTP direction of weight change, capturing the function of CaMKII in the Kinase learning rule.`, 22: `LearnCaD is the cascaded integration of [LearnCaP] using the DTau time constant (typically 40), representing the minus, LTD direction of weight change, capturing the function of DAPK1 in the Kinase learning rule.`, 23: `CaDiff is difference between [LearnCaP] - [LearnCaD]. This is the error signal that drives error-driven learning.`, 24: `CaDiffAvg is the running time-average of |CaDiff| (absolute value), used for determining the timing of learning in terms of onsets of peaks. See [MinusPeak] and [PlusPeak].`, 25: `CaDiffPeak is the value of the current peak (local maximum) of [CaDiffAvg].`, 26: `CaDiffPeakCyc is the absolute cycle where [CaDiffPeak] occurred.`, 27: `MinusPeak is the value of the first, minus-phase peak of [CaDiffAvg], which occurs when new input drives the fast integral to diverge from slow.`, 28: `MinusPeakCyc is the absolute cycle where [MinusPeak] occurred.`, 29: `PlusPeak is the value of the second, plus-phase peak of [CaDiffAvg], which occurs when an outcome causes fast integral to diverge from slow.`, 30: `PlusPeakCyc is the absolute cycle where [PlusPeak] occurred.`, 31: `LearnDiff is the actual difference signal that drives learning, which is computed from [CaDiff] for neocortical neurons, but specifically at the point of learning (LearnNow), based on [PlusPeakCyc]. It is cleared at the start of a new learning window.`, 32: `LearnNow is activated at the moment when the receiving neuron is learning, based on timing computed from [MinusPeak] and [PlusPeak]. See [LearnTimingParams].`, 33: `RLRate is recv-unit based learning rate multiplier, reflecting the sigmoid derivative computed from [CaD] of recv unit, and the normalized difference (CaP - CaD) / MAX(CaP - CaD).`, 34: `ETrace is the eligibility trace for this neuron.`, 35: `ETrace is the learning factor for the eligibility trace for this neuron.`, 36: `GnmdaSyn is the integrated NMDA synaptic current on the receiving neuron. It adds GeRaw and decays with a time constant.`, 37: `Gnmda is the net postsynaptic (receiving) NMDA conductance, after Mg V-gating and Gbar. This is added directly to Ge as it has the same reversal potential.`, 38: `GnmdaLrn is learning version of integrated NMDA recv synaptic current. It adds [GeRaw] and decays with a time constant. This drives [NmdaCa] that then drives [LearnCa] for learning.`, 39: `GnmdaMaint is net postsynaptic maintenance NMDA conductance, computed from [GMaintSyn] and [GMaintRaw], after Mg V-gating and Gbar. This is added directly to Ge as it has the same reversal potential.`, 40: `NmdaCa is NMDA calcium computed from GnmdaLrn, drives learning via CaM.`, 41: `Gvgcc is conductance (via Ca) for VGCC voltage gated calcium channels.`, 42: `VgccM is activation gate of VGCC channels.`, 43: `VgccH inactivation gate of VGCC channels.`, 44: `VgccCa is the instantaneous VGCC calcium flux: can be driven by spiking or directly from Gvgcc.`, 45: `VgccCaInt is the time-integrated VGCC calcium flux. This is actually what drives learning.`, 46: `Burst is the layer 5 IB intrinsic bursting neural activation value, computed by thresholding the [CaP] value in Super superficial layers.`, 47: `BurstPrv is previous Burst bursting activation from prior time step. Used for context-based learning.`, 48: `CtxtGe is context (temporally delayed) excitatory conductance, driven by deep bursting at end of the plus phase, for CT layers.`, 49: `CtxtGeRaw is raw update of context (temporally delayed) excitatory conductance, driven by deep bursting at end of the plus phase, for CT layers.`, 50: `CtxtGeOrig is original CtxtGe value prior to any decay factor. Updates at end of plus phase.`, 51: `GgabaB is net GABA-B conductance, after Vm gating and Gk + Gbase. Applies to Gk, not Gi, for GIRK, with .1 reversal potential.`, 52: `GababM is the GABA-B / GIRK activation, which is a time-integrated value with rise and decay time constants.`, 53: `GababX is GABA-B / GIRK internal drive variable. This gets the raw activation and decays.`, 54: `Gak is the conductance of A-type K potassium channels.`, 55: `SSGiDend is the amount of SST+ somatostatin positive slow spiking inhibition applied to dendritic Vm (VmDend).`, 56: `GknaMed is the conductance of sodium-gated potassium channel (KNa) medium dynamics (Slick), which produces accommodation / adaptation.`, 57: `GknaSlow is the conductance of sodium-gated potassium channel (KNa) slow dynamics (Slack), which produces accommodation / adaptation.`, 58: `Gkir is the conductance of the potassium (K) inwardly rectifying channel, which is strongest at low membrane potentials. Can be modulated by DA.`, 59: `KirM is the Kir potassium (K) inwardly rectifying gating value.`, 60: `Gsk is Calcium-gated potassium channel conductance as a function of Gbar * SKCaM.`, 61: `SKCaIn is intracellular calcium store level, available to be released with spiking as SKCaR, which can bind to SKCa receptors and drive K current. replenishment is a function of spiking activity being below a threshold.`, 62: `SKCaR is the released amount of intracellular calcium, from SKCaIn, as a function of spiking events. This can bind to SKCa channels and drive K currents.`, 63: `SKCaM is the Calcium-gated potassium channel gating factor, driven by SKCaR via a Hill equation as in chans.SKPCaParams.`, 64: `Gmahp is medium time scale AHP conductance.`, 65: `MahpN is accumulating voltage-gated gating value for the medium time scale AHP.`, 66: `Gsahp is slow time scale AHP conductance.`, 67: `SahpCa is slowly accumulating calcium value that drives the slow AHP.`, 68: `SahpN is the sAHP gating value.`, 69: `ActM is ActInt activation state at end of third quarter, representing the posterior-cortical minus phase activation. This is used for statistics and monitoring network performance. Should not be used for learning or other computations.`, 70: `ActP is ActInt activation state at end of fourth quarter, representing the posterior-cortical plus_phase activation. This is used for statistics and monitoring network performance. Should not be used for learning or other computations.`, 71: `Beta1 is the activation state at the first beta cycle within current state processing window (i.e., at 50 msec), as saved by Beta1() function. Used for example in hippocampus for CA3, CA1 learning.`, 72: `Beta2 is the activation state at the second beta cycle within current state processing window (i.e., at 100 msec), as saved by Beta2() function. Used for example in hippocampus for CA3, CA1 learning.`, 73: `CaPMax is the maximum [CaP] across one theta cycle time window (max of CaPMaxCa). It is used for specialized algorithms that have more phasic behavior within a single trial, e.g., BG Matrix layer gating. Also useful for visualization of peak activity of neurons.`, 74: `CaPMaxCa is the Ca integrated like [CaP] but only starting at the MaxCycStart cycle, to prevent inclusion of carryover spiking from prior theta cycle trial. The PTau time constant otherwise results in significant carryover. This is the input to CaPMax.`, 75: `GeNoise is integrated noise excitatory conductance, added into Ge.`, 76: `GeNoiseP is accumulating poisson probability factor for driving excitatory noise spiking. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda as function of noise firing rate.`, 77: `GiNoise is integrated noise inhibotyr conductance, added into Gi.`, 78: `GiNoiseP is accumulating poisson probability factor for driving inhibitory noise spiking. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda as a function of noise firing rate.`, 79: `GeExt is extra excitatory conductance added to Ge, from Ext input, GeCtxt etc.`, 80: `GeRaw is the raw excitatory conductance (net input) received from senders = current raw spiking drive.`, 81: `GeSyn is the time-integrated total excitatory (AMPA) synaptic conductance, with an instantaneous rise time from each spike (in GeRaw) and exponential decay with Dt.GeTau, aggregated over pathways. Does *not* include Gbar.E.`, 82: `GiRaw is the raw inhibitory conductance (net input) received from senders = current raw spiking drive.`, 83: `GiSyn is time-integrated total inhibitory synaptic conductance, with an instantaneous rise time from each spike (in GiRaw) and exponential decay with Dt.GiTau, aggregated over pathways -- does *not* include Gbar.I. This is added with computed FFFB inhibition to get the full inhibition in Gi.`, 84: `GeInt is integrated running-average activation value computed from Ge with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall Ge level across the ThetaCycle time scale (Ge itself fluctuates considerably). This is useful for stats to set strength of connections etc to get neurons into right range of overall excitatory drive.`, 85: `GeIntNorm is normalized GeInt value (divided by the layer maximum). This is used for learning in layers that require learning on subthreshold activity.`, 86: `GiInt is integrated running-average activation value computed from GiSyn with time constant Act.Dt.IntTau, to produce a longer-term integrated value reflecting the overall synaptic Gi level across the ThetaCycle time scale (Gi itself fluctuates considerably). Useful for stats to set strength of connections etc to get neurons into right range of overall inhibitory drive.`, 87: `GModRaw is raw modulatory conductance, received from GType = ModulatoryG pathways.`, 88: `GModSyn is syn integrated modulatory conductance, received from GType = ModulatoryG pathways.`, 89: `SMaintP is accumulating poisson probability factor for driving self-maintenance by simulating a population of mutually interconnected neurons. Multiply times uniform random deviate at each time step, until it gets below the target threshold based on poisson lambda based on accumulating self maint factor.`, 90: `GMaintRaw is raw maintenance conductance, received from GType = MaintG pathways.`, 91: `GMaintSyn is syn integrated maintenance conductance, integrated using MaintNMDA params.`, 92: `NeurFlags are bit flags for binary state variables, which are converted to / from uint32. These need to be in Vars because they can be differential per data (for ext inputs) and are writable (indexes are read only).`, 93: `CaBins is a vector of values starting here, with aggregated [CaSyn] values in time bins of [Context.CaBinCycles] across the theta cycle, for computing synaptic calcium efficiently. Each bin = Sum(CaSyn) / CaBinCycles. Total number of bins = [Context.ThetaCycles] / CaBinCycles. Synaptic calcium is integrated from sender * receiver CaBins values, with weights for CaP vs CaD that reflect their faster vs. slower time constants, respectively. CaD is used for the credit assignment factor, while CaP - CaD is used directly for error-driven learning at Target layers.`}

var _NeuronVarsMap = map[NeuronVars]string{0: `Spike`, 1: `Spiked`, 2: `Act`, 3: `ActInt`, 4: `Ge`, 5: `Gi`, 6: `Gk`, 7: `Inet`, 8: `Vm`, 9: `VmDend`, 10: `ISI`, 11: `ISIAvg`, 12: `Ext`, 13: `Target`, 14: `CaM`, 15: `CaP`, 16: `CaD`, 17: `CaDPrev`, 18: `CaSyn`, 19: `LearnCa`, 20: `LearnCaM`, 21: `LearnCaP`, 22: `LearnCaD`, 23: `CaDiff`, 24: `CaDiffAvg`, 25: `CaDiffPeak`, 26: `CaDiffPeakCyc`, 27: `MinusPeak`, 28: `MinusPeakCyc`, 29: `PlusPeak`, 30: `PlusPeakCyc`, 31: `LearnDiff`, 32: `LearnNow`, 33: `RLRate`, 34: `ETrace`, 35: `ETraceLearn`, 36: `GnmdaSyn`, 37: `Gnmda`, 38: `GnmdaLrn`, 39: `GnmdaMaint`, 40: `NmdaCa`, 41: `Gvgcc`, 42: `VgccM`, 43: `VgccH`, 44: `VgccCa`, 45: `VgccCaInt`, 46: `Burst`, 47: `BurstPrv`, 48: `CtxtGe`, 49: `CtxtGeRaw`, 50: `CtxtGeOrig`, 51: `GgabaB`, 52: `GababM`, 53: `GababX`, 54: `Gak`, 55: `SSGiDend`, 56: `GknaMed`, 57: `GknaSlow`, 58: `Gkir`, 59: `KirM`, 60: `Gsk`, 61: `SKCaIn`, 62: `SKCaR`, 63: `SKCaM`, 64: `Gmahp`, 65: `MahpN`, 66: `Gsahp`, 67: `SahpCa`, 68: `SahpN`, 69: `ActM`, 70: `ActP`, 71: `Beta1`, 72: `Beta2`, 73: `CaPMax`, 74: `CaPMaxCa`, 75: `GeNoise`, 76: `GeNoiseP`, 77: `GiNoise`, 78: `GiNoiseP`, 79: `GeExt`, 80: `GeRaw`, 81: `GeSyn`, 82: `GiRaw`, 83: `GiSyn`, 84: `GeInt`, 85: `GeIntNorm`, 86: `GiInt`, 87: `GModRaw`, 88: `GModSyn`, 89: `SMaintP`, 90: `GMaintRaw`, 91: `GMaintSyn`, 92: `NeurFlags`, 93: `CaBins`}
