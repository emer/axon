...
func TestActUpdate(t *testing.T) {
	...
	for i := range geinc {
		Neurons.SetAdd(geinc[i], int(GeRaw), int(ni), int(di))
		Neurons.Set(ac.Dt.GeSynFromRaw(Neurons.Value(int(GeSyn), int(ni), int(di)), Neurons.Value(int(GeRaw), int(ni), int(di))), int(GeSyn), int(ni), int(di))
		ac.GeFromSyn(ctx, ni, di, Neurons.Value(int(GeSyn), int(ni), int(di)), Neurons.Value(int(GeExt), int(ni), int(di)))
		ac.GiFromSyn(ctx, ni, di, Neurons.Value(int(GiSyn), int(ni), int(di)))
		ac.VmFromG(ctx, ni, di)
		ac.SpikeFromVm(ctx, ni, di)
		ge[i] = Neurons.Value(int(Ge), int(ni), int(di))
		inet[i] = Neurons.Value(int(Inet), int(ni), int(di))
		vm[i] = Neurons.Value(int(Vm), int(ni), int(di))
		spike[i] = Neurons.Value(int(Spike), int(ni), int(di))
		act[i] = Neurons.Value(int(Act), int(ni), int(di))
