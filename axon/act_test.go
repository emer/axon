...
func TestActUpdate(t *testing.T) {
	...
	for i := range geinc {
		Neurons.SetAdd(geinc[i], int(ni), int(GeRaw), int(di))
		Neurons.Set(ac.Dt.GeSynFromRaw(Neurons.Value(int(ni), int(GeSyn), int(di)), Neurons.Value(int(ni), int(GeRaw), int(di))), int(ni), int(GeSyn), int(di))
		ac.GeFromSyn(ctx, ni, di, Neurons.Value(int(ni), int(GeSyn), int(di)), Neurons.Value(int(ni), int(GeExt), int(di)))
		ac.GiFromSyn(ctx, ni, di, Neurons.Value(int(ni), int(GiSyn), int(di)))
		ac.VmFromG(ctx, ni, di)
		ac.SpikeFromVm(ctx, ni, di)
		ge[i] = Neurons.Value(int(ni), int(Ge), int(di))
		inet[i] = Neurons.Value(int(ni), int(Inet), int(di))
		vm[i] = Neurons.Value(int(ni), int(Vm), int(di))
		spike[i] = Neurons.Value(int(ni), int(Spike), int(di))
		act[i] = Neurons.Value(int(ni), int(Act), int(di))
