...
void MinusPhaseNeuron2(in Context ctx, in LayerParams ly, uint ni, uint di, in Pool pl) {
	ly.MinusPhaseNeuron(ctx, ni, di, pl, Pools[ly.Indexes.PoolIndex(0, di)], LayValues[ly.Indexes.ValuesIndex(di)]);
}

void MinusPhaseNeuron(in Context ctx, uint ni, uint di) {
	uint li = NrnI(ctx, ni, NrnLayIndex);
	uint pi = NrnI(ctx, ni, NrnSubPool);
	MinusPhaseNeuron2(ctx, Layers[li], ni, di, Pools[Layers[li].Indexes.PoolIndex(pi, di)]);
}
