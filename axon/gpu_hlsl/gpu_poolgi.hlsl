...
void PoolGi2(in Context ctx, in LayerParams ly, uint pi, uint di, inout Pool pl, float giMult) {
	if(pl.IsLayPool == 0) {
...
void PoolGi(in Context ctx, uint pi, uint di, inout Pool pl) {
	PoolGi2(ctx, Layers[pl.LayIdx], pi, di, pl, LayVals[ctx.NetIdxs.ValsIdx(pl.LayIdx, di)].ActAvg.GiMult);
}
