...
void PostSpikeLay(in Context ctx, in LayerParams ly, uint ni, uint di, in Pool pl, in Pool lpl, inout LayerValues vals) {
...
void PostSpike2(in Context ctx, LayerParams ly, uint ni, uint di) {
	...
	PostSpikeLay(ctx, ly, ni, di, Pools[ly.Indexes.PoolIndex(pi, di)], Pools[ly.Indexes.PoolIndex(0, di)], LayValues[ly.Indexes.ValuesIndex(di)]);
}
