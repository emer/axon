...
func (ly *TRCLayer) SetDriverNeuron(tni int, drv float32) {
		...
		if dly.Is2D() {
			if ly.Is2D() {
				for dni := range dly.Neurons {
					...
					ly.SetDriverNeuron(tni, ly.TRC.GeFmMaxAvg(drvAct, drvAct))
				}
				...
				for dni := range dly.Neurons {
					...
					for py := 0; py < pyn; py++ {
						for px := 0; px < pxn; px++ {
							...
							ly.SetDriverNeuron(pni, ly.TRC.GeFmMaxAvg(drvAct, drvAct))
						}
					}
				}
			}
			...
			if ly.Is2D() {
				for dni := 0; dni < dnun; dni++ {
					...
					tni := drv.Off + dni
					ly.SetDriverNeuron(tni, ly.TRC.GeFmMaxAvg(max, avg))
				}
